...
Page({
    ...
    onLoad(options) {
        ...
        const cachedDay = globalData.currentDayData;
        const cachedIndex = globalData.currentDayIndex;
    ...
    async loadDay(planId, dayIndex) {
        try {
            const plan = await (0, request_1.get)(`/execution/${planId}`);
