...
# ── 路由顺序修复（P1 路由遮蔽）──
# 确保 POST /diagnosis/start 在 POST /diagnosis/{business_id} 之前匹配，
# 避免 Starlette 将 "start" 当作 business_id 动态参数截获。
_fixed_routes: list = []
for _r in router.routes:
    if hasattr(_r, "path") and _r.path == "/diagnosis/start":
        _fixed_routes.insert(0, _r)
    else:
        _fixed_routes.append(_r)
router.routes = _fixed_routes
