...
import { TokenStorage, clearDevAndLegacyCaches, storageSelfCheckReport } from './utils/storage'
import { DEV_DEFAULT_URL } from './config'

App({
  globalData: {
    /** Dev 环境地址（局域网 IP，config.ts 统一管理） */
    DEV_API_BASE: DEV_DEFAULT_URL,
  ...
  onUnhandledRejection(err: WechatMiniprogram.OnUnhandledRejectionCallbackResult): void {
    console.error('[app] 未处理的 Promise 拒绝：', err?.reason)
  },

  /** 路由到不存在的页面时给出提示，避免静默白屏 */
  onPageNotFound(res: WechatMiniprogram.OnPageNotFoundCallbackResult): void {
  ...
  onLaunch(): void {
    try {
      const cleared = clearDevAndLegacyCaches()
      ...
      if (this.isDev()) storageSelfCheckReport()
    } catch (err) {
