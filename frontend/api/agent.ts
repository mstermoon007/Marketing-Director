...
import { get, post, stream, upload, StreamHandle } from './request'
...
export function streamChat(opts: StreamChatOptions): StreamHandle {
  return stream({
    ...
    onEvent: (e) => opts.onEvent(e as AgentStreamEvent),
 ...
 * 读取某会话历史。
 */
export function getAgentHistory(sessionId: string): Promise<AgentHistory> {
  return get<AgentHistory>('/agent/history', { session_id: sessionId })
}
 ...
 */
export function uploadReviewFiles(planId: string, files: string[]): Promise<unknown> {
  return upload(`/review/${planId}`, files)
}
